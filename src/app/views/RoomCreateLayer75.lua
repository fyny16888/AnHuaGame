...
function RoomCreateLayer:onCreate(parameter)
    ...
    local csb = cc.CSLoader:createNode("RoomCreateLayer75.csb")
    self:addChild(csb)
    ...
    --查叫
    if self.recordCreateParameter["bChaJiao"] ~= nil and self.recordCreateParameter["bChaJiao"] == 1 then
        items[1]:setBright(true)
        local uiText_desc = ccui.Helper:seekWidgetByName(items[1],"Text_desc")
        if uiText_desc ~= nil then 
            uiText_desc:setTextColor(cc.c3b(238,105,40))
    ...
    --上楼
    if self.recordCreateParameter["bShangLou"] ~= nil and self.recordCreateParameter["bShangLou"] == 1 then
        items[2]:setBright(true)
        local uiText_desc = ccui.Helper:seekWidgetByName(items[2],"Text_desc")
        if uiText_desc ~= nil then 
            uiText_desc:setTextColor(cc.c3b(238,105,40))
    ...
    Common:addCheckTouchEventListener(items)
    if self.recordCreateParameter["bFengDing"] ~= nil and self.recordCreateParameter["bFengDing"] == 16 then
        items[2]:setBright(true)
        local uiText_desc = ccui.Helper:seekWidgetByName(items[2],"Text_desc")
        if uiText_desc ~= nil then 
            uiText_desc:setTextColor(cc.c3b(238,105,40))
...
function RoomCreateLayer:onEventCreate(nTableType)
    ...
    local items = ccui.Helper:seekWidgetByName(uiListView_parameterList:getItem(2),"ListView_parameter"):getItems()
    if items[1]:isBright() then   	-- BYTE bPinDao;				//频道     0.全频道   1.半频道
        tableParameter.bChaJiao = 1
    else
        tableParameter.bChaJiao = 0
    end

    if items[2]:isBright() then   	-- BYTE bPinDao;				//频道     0.全频道   1.半频道
        tableParameter.bShangLou = 1
    else
        tableParameter.bShangLou = 0
    end
    ...
    if items[1]:isBright() then
        ...
        tableParameter.bFengDing = 16
    end
   
    --默认选项
    tableParameter.bGaiPai = 0          -- BYTE bGaiPai;				//盖牌	   0.关闭     1.开启
    -- tableParameter.bChaJiao = 0	        -- BYTE bChaJiao;				//查叫	   0.关闭     1.开启
    -- tableParameter.bShangLou = 0    	-- BYTE bShangLou;				//上楼	   0.关闭     1.开启
    tableParameter.bPaoQiaMoBa = 0  	-- BYTE bPaoQiaMoBa;			//跑恰摸八 0.关闭     1.开启
