...
--   聊天
---------------
local PDKChat = class("PDKChat", cc.load("mvc").ViewBase)
...
function PDKChat:onCreate(params)
	local wKindID = params[1]
	if wKindID == 85 then
...
function PDKChat:initEmSetting(...)
	--50 为了做分页给每页表情赋值一个id
	self:initOneEmjio('page1_Button_1', 'page1_ScrollView_1', 'press_1', 23, 0)
end

--初始化表情 --page1_ScrollView_1
function PDKChat:initOneEmjio(btnName, listName, press, index, start)
...
function PDKChat:addMaskListen(...)
...
function PDKChat:addPanelListen(item, call)
...
function PDKChat:closeView(...)
	self:setVisible(false)
end

function PDKChat:onSendCall(...)
	local contents = ''
	if self.isOS then
	...
	PDKGameCommon:getRoleChairID(), 0, PDKGameCommon:getUserInfo(PDKGameCommon:getRoleChairID()).cbSex, 32, "", string.len(contents), string.len(contents), contents)
	self:setVisible(false)
...
function PDKChat:clickExpressLab(sender)
	...
	NetMgr:getGameInstance():sendMsgToSvr(NetMsgId.MDM_GR_USER, NetMsgId.REQ_GR_USER_SEND_CHAT, "dwbnsdns",
	PDKGameCommon:getRoleChairID(), index, PDKGameCommon:getUserInfo(PDKGameCommon:getRoleChairID()).cbSex, 32, "", string.len(contents), string.len(contents), contents)
	self:removeFromParent()
end

function PDKChat:onClickHead(sender)
	local name = sender:getName()
	
	self:showPage(name)
end

function PDKChat:showPage( hedeName )
...
function PDKChat:buttonCall(sender)
	local index = sender:getName()
	self:hidePage(self.page_1)
	NetMgr:getGameInstance():sendMsgToSvr(NetMsgId.MDM_GF_GAME, NetMsgId.SUB_GF_USER_EXPRESSION, "ww", index, PDKGameCommon:getRoleChairID())
	self:removeFromParent()
end

function PDKChat:emButtonCall(sender)
	local name = sender:getName()
	self:showPage(name)
end


function PDKChat:addListener(btn, callback)
...
function PDKChat:SUB_GR_SEND_CHAT(event)

end

function PDKChat:SUB_GF_USER_EXPRESSION(event)
