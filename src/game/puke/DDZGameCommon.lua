...
local DDZGameCommon = 
{
    ...
    CardType_pair           = 2,    --对子
    CardType_three          = 3,    --三条类型
    CardType_straight       = 4,    --顺子
    CardType_straightPair   = 5,    --连对    
    CardType_airplane       = 6,    --飞机
    CardType_3Add1          = 7,    --三带一
    CardType_3Add2          = 8,    --三带二
    CardType_4Add2          = 9,    --四带二
    CardType_4Add3          = 10,    --四带三
    CardType_ruanbomb		= 11,	 --软炸弹
    CardType_bomb           = 12,    --炸弹
    CardType_laizibomb      = 13,   --赖子炸弹
    CardType_missilebomb    = 14,      --火箭类型
...
function DDZGameCommon:getViewIDByChairID(wChairID)
...
function DDZGameCommon:getRoleChairID()
    return self.meChairID
end

function DDZGameCommon:ContinueGame(cbLevel)
    NetMgr:getGameInstance():sendMsgToSvr(NetMsgId.MDM_GR_USER,NetMsgId.REQ_GR_USER_SET_POSITION,"aad",LocationSystem.pos.x, LocationSystem.pos.y, DDZGameCommon.dwUserID)
...
function DDZGameCommon:getUserInfoByUserID(dwUserID)
    for key, var in pairs(self.player) do
...
function DDZGameCommon:rad(d)
    return d* math.pi / 180.0;
end 

function DDZGameCommon:GetDistance(lat1,lat2)
...
--牌资源
function DDZGameCommon:getCardNode(data)
...
function DDZGameCommon:playAnimation(root,id, wChairID)
    local Animation = nil
    if DDZGameCommon.tableConfig.wKindID == 51 or DDZGameCommon.tableConfig.wKindID == 55 or DDZGameCommon.tableConfig.wKindID == 56 or DDZGameCommon.tableConfig.wKindID == 57 or DDZGameCommon.tableConfig.wKindID == 58 or DDZGameCommon.tableConfig.wKindID == 59 then
        Animation = require("game.puke.AnimationNiu")
    elseif DDZGameCommon.tableConfig.wKindID == 53 then
    ...
    local AnimationData = Animation[id][DDZGameCommon.regionSound]
    ...
    if AnimationData.animFile ~= "" then
        if id == "报警" then
            ...
            if DDZGameCommon.tableConfig.wKindID == 51 or DDZGameCommon.tableConfig.wKindID == 53 or DDZGameCommon.tableConfig.wKindID == 55 or DDZGameCommon.tableConfig.wKindID == 56 or DDZGameCommon.tableConfig.wKindID == 57 or DDZGameCommon.tableConfig.wKindID == 58 or DDZGameCommon.tableConfig.wKindID == 59 then
    ...
    if wChairID ~= nil then
        soundFile = AnimationData.sound[DDZGameCommon.player[wChairID].cbSex]
    else
        soundFile = AnimationData.sound[0]
    end
    if soundFile ~= "" then
        if id ~= "我先出" or wChairID == self:getRoleChairID()  then 
            require("common.Common"):playEffect(AnimationData.sound[DDZGameCommon.player[wChairID].cbSex])
        end  
    end
