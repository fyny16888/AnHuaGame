...
local KwxLocationLayer       = class("KwxLocationLayer", cc.load("mvc").ViewBase)

function KwxLocationLayer:onConfig()
    self.widget             = {
        {"Image_distance2"},
        {"Image_distance3"},
        {"Image_distance4"},
...
function KwxLocationLayer:onEnter()
...
function KwxLocationLayer:onExit()
...
function KwxLocationLayer:onCreate(params)
    self.isDistance = params[1] --是否是距离报警
    self:refreshUI()
end

function KwxLocationLayer:onQuit( ... )
...
function KwxLocationLayer:onClose()
    self:removeFromParent()
end
...
--                            game logic                              --
------------------------------------------------------------------------
function KwxLocationLayer:refreshUI()
...
function KwxLocationLayer:showPlayerPosition(rootNode, playerNum)
...
function KwxLocationLayer:RET_GAMES_USER_POSITION(event)
    local playerNum = GameCommon.gameConfig.bPlayerCount
    local rootNode = self['Image_distance' .. playerNum]
    rootNode:setVisible(true)
    self:showPlayerPosition(rootNode, playerNum)
...
function KwxLocationLayer:USER_LEAVETABLE( ... )
    if self.isDistance then
        self:removeFromParent()
    end
end

return KwxLocationLayer